...
def overall_stats():
  ...
  # find the times
  with open('dmm_unroll.csv') as csv_file:
    csv_reader = csv.reader(csv_file, delimiter=',')
    line_count = 0
    for row in csv_reader:
      if (line_count % 2 == 1): 
        highest_freq.append((row[4], row[5], row[6]))
        execution_time = (float(row[3].split(":")[2])/1000)
        if (row[2] == "True"):
  ...
  i_core = []
  power_core = []
  with open('dmm_unroll_core.csv') as csv_file:
    csv_reader = csv.reader(csv_file, delimiter=',')
    line_count = 0
    '''
    example:
  ...
  i_mem = []
  power_mem = []
  with open('dmm_unroll_mem.csv') as csv_file:
    csv_reader = csv.reader(csv_file, delimiter=',')
    line_count = 0
    '''
    example:
  ...
  # "#7B2D43", "#59B217"
  print("min_energy_lst - ", min_energy_lst)
  ...
  ax.set(xlim=(0, 100), ylim=(0, 100), xlabel="Frequency (in MHz)", ylabel="Energy (in uJ)")
  ...
  for j in range(len(total_power_lst)):
    total_power_lst[j] = total_power_lst[j]*10**3
