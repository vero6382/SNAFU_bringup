...
def find_min_energy():
  ...
  with open('../dconv_unroll.csv') as csv_file:
    ...
    for row in csv_reader:
      if (row[2] == "True"): 
        successful += 1
        temp_time += float(row[3].split(":")[2])/3
      if (line % 3 == 2):
        if (successful > 0):
  ...
  for j in np.arange(0.60, 0.96, 0.02):
    for i in range(20, 150, 10):
      ...
      # print("configs = {} and execution_time = {}".format((i, volt), times[ind]))
      ind += 1

  totalV = 0
  totalA = 0

  with open('../dconv_unroll_repeat_core.csv') as csv_file:
    csv_reader = csv.reader(csv_file, delimiter=',')
    line = 0
    for row in csv_reader:
  ...
  with open('../dconv_unroll_repeat_mem.csv') as csv_file:
    csv_reader = csv.reader(csv_file, delimiter=',')
    line = 0
    for row in csv_reader:
      if 'voltage (v)' not in row:
